...
func testStress(ns, qps int, duration time.Duration) StressResult {
	if ns < 2 {
	...
	// Create a network of ns DHT nodes
	...
	defer n.Close()
	stressor, err := n.AddHost("stressor")
	if err != nil {
		panic(err)
	}

	cidr := "10.42.0.0/16"
	idx := stressor.Interfaces[0].Link.Attrs().Index
	_, prefix, err := net.ParseCIDR(cidr)
	if err != nil {
		panic(err)
	}
	err = stressor.AddRoute(&netlink.Route{
		LinkIndex: idx,
		Scope:     unix.RT_SCOPE_HOST,
		Dst:       prefix,
		Protocol:  unix.RTPROT_BOOT,
	...
	ctlData, err := json.Marshal(stCtl)
	if err != nil {
		panic(err)
	}

	out := new(bytes.Buffer)
	_, err = stressor.RunGo("./inuctl", cmdopt.Args("stress"), coptCtx,
		cmdopt.Stdin(bytes.NewReader(ctlData)), cmdopt.Stderr(os.Stderr), cmdopt.Stdout(out))
	if err != nil {
		panic(err)
	}
